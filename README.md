# LeafFlow 是什么？
LeafFlow (leaf², a **l**ightweight **ea**sy **f**ast and **f**lexible execution framework) 是一个轻量简洁的执行框架，可以快速灵活地为命令行程序提供一个脉络清晰的UI。

## 动机
许多程序仅提供命令行参数的交互方式，但有个用户界面明显更方便，Gradio/ComfyUI都是常见的选择，但是：
- **Gradio**：开发快但启动慢，不灵活，每个程序单独开发，依赖python
- **ComfyUI**：很灵活但略显复杂，学习成本高

个人而言，大多数时候只想赶紧把推理跑起来，没有心情去安装Gradio再等它加载，也懒得摆弄ComfyUI的节点。思来想去打算做一个开源、统一的UI，方便地调整、运行程序 —— 这就是Leaf。

leaf² 是Gradio/ComfyUI的折中，核心上可能更接近于ComfyUI：
- ✨ 像 Gradio 一样简单直观的表单界面
- ⚡ 轻量快速，无重度依赖
- 🎯 统一管理多个程序（CLI/TUI）
- 🔧 类似 ComfyUI 的简单工作流
...
leaf²以yaml的形式组织各种命令行程序（CLI、TUI）的参数，并据此构建相应的UI。通过UI调整、渲染出可执行命令并发送到调度器执行
...
# 如何使用
## 安装
1. 直接在release下载zip解压（只有windows）
2. 自行从源码编译（理论上支持linux，但没测试）

## 蓝图
参考下方 `如何编写bud？` 一节以及 [project示例](./bud/leaf/demo.yaml), [flow示例](./bud/sprig/demo.flow.yaml) 这两个标准示例进行编写。   
虽然内容挺多，但不妨交给ai再人工微调  
...
# 如何编写bud？
蓝图，或者说嫩芽（`bud`），一个能被leaf²解析的yaml文件。细分为叶片（`leaf`）（对应前述project/“项目”）, 枝条（`sprig`）（对应flow/“流”）两种：
